...
- Twee zonnen: de **render-zon** (`extra.zonPos`; schaduwkaart, `_extra[8..10]`) is los van de **sim-zon** (`reken.zonRicht`, per frame uit `_jaarTeller` (jaar = 4000 frames, start in de winter; declinatie = `−askanteling·cos(jaarhoek)`) en `_zonSlotTeller` (dagomloop = 360 frames)). `zonSchijn` berekent per cel de invalsfactor × een horizon-versperring die over het grid naar de zon-azimut wandelt (géén schaduwkaart in compute meer). B-toets/sleep op de achtergrond veranderen alleen `_zonPos`: de sim is bit-identiek met/zonder `--zonder-schaduw`.
- Render-pass-volgorde (`doeRenderPassen`): wolken(geflipt, `cull Front`) → grond → ijs-onderkant(`cull Front`) → ijs-bovenkant(`cull Back`) → water → wolken(normaal, `cull Back`) → highlight → GUI. Eerste pass wist kleur+diepte (`wisScherm`); ijs heeft eigen shaders (`planeetgridVertIjs[Onder].wgsl` + gedeelde `planeetgridFragIjs.wgsl`) en tekent ijs drijvend op de waterspiegel (`waterSchijn`) met zijn echte dikte — niet zoals de sim het intern als "grond onder water" rekent. Beide ijs-passes schrijven diepte vóór het water, zodat het transparante water ijs alleen bedekt waar het er vóór ligt (waterbulten boven het ijs correct). Het water zelf schrijft óók diepte (`LessEqual`): overlappende wateroppervlakken (bulten) sorteren zo op ware diepte i.p.v. driehoek-volgorde.
