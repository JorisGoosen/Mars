...
- Render-pass-volgorde (`doeRenderPassen`): wolken(geflipt, `cull Front`) → grond → water → ijs(onder `cull Front`, dan boven `cull Back`) → wolken(normaal, `cull Back`) → highlight → GUI. Eerste pass wist kleur+diepte (`wisScherm`); ijs heeft eigen shaders (`planeetgridVertIjs[Onder].wgsl` + gedeelde `planeetgridFragIjs.wgsl`) en tekent ijs drijvend op de waterspiegel (`waterSchijn`) met zijn echte dikte — niet zoals de sim het intern als "grond onder water" rekent.
